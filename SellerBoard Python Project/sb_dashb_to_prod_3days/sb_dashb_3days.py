...
def last_3days_period():

    current_date = datetime.now(timezone.utc)

    start_date = (current_date - timedelta(days=3)).replace(hour=0, minute=0, second=0, microsecond=0)

    end_date = (current_date - timedelta(days=1)).replace(hour=23, minute=59, second=59, microsecond=0)
...
def delete_rows_1_2_3_days_ago(spreadsheet_id):
    ...
    one_days_ago = (now - timedelta(days=1)).strftime("%m/%d/%Y").lstrip('0').replace('/0', '/')
    two_days_ago = (now - timedelta(days=2)).strftime("%m/%d/%Y").lstrip('0').replace('/0', '/')
    three_days_ago = (now - timedelta(days=3)).strftime("%m/%d/%Y").lstrip('0').replace('/0', '/')

    rows_to_delete = []

    for index, row in enumerate(all_rows):
        if row[2] in {today, one_days_ago, two_days_ago, three_days_ago}:
    ...
    print("Lines with today, one, two, three ago dates have been removed.")


def main():

    base_dir = "../sb_dashb_to_prod_3days"
    cookies_file = "3days_cookies.json"

    main_frame(base_dir=base_dir, cookies_file=cookies_file,
               delete_rows_func=delete_rows_1_2_3_days_ago, export_period_func=last_3days_period,
